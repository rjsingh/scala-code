...
class NonEmpty(var elem: Tweet, var left: TweetSet, var right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted(): Tweet = {
    if(this.right.isEmpty) this.elem
    else this.right.mostRetweeted
  }
  ...
  override def descendingByRetweet(): TweetList = {
    if (this.isEmpty) Nil
      ...
      this.remove(x)
      new Cons(x, this.descendingByRetweet())
    }
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter((t: Tweet) => {google.exists((s: String) => {s.contains(t.text.toCharArray())})})
  }
  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter((t: Tweet) => {apple.exists((s: String) => {s.contains(t.text.toCharArray())})})
  }
